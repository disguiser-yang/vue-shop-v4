<template>
  <div>
     ...
     <div class="item title_d"><h5>{{ this.goodsInfo.goods_name }}</h5>
      <span class="shop_price red">￥{{ this.goodsInfo.goods_price }}</span>
      <span class='goods_jingle'>{{ this.goodsInfo.title }}</span>
      <p class="market_price gray">价格<span>￥{{ this.goodsInfo.goods_marketprice }}</span></p>
      <p class='goods_hair_info gray'>
        <span>快递：{{this.goodsHairInfo.content}}
        </span>
        <span>销量：{{this.goodsInfo.goods_salenum}}
        </span>
        <span>{{this.goodsHairInfo.area_name}}
        </span>
      </p>
     </div>
     <div class='item_goods_discribe' v-on:click='showDiscribe'>
        产品参数<img src='../../assets/images/productpage_arrow@2x.png'>
     </div>
     <div class='item_goods_hair_info'>
     ...
     <div class='item_describe_info_mask' v-on:click='hideDiscribe' v-show='pullUp'>
    
     </div>
     <div class='item_describe_info' v-bind:class='{ mask_up : pullUp ,mask_down : !pullUp}'>
        <p>产品参数</p>
        <scroller lock-x height="300px" >
          <div class="scroll_box">
              <div class='scroll_box_info clearfix' v-for='valueObj in this.goodsInfo.goods_attr'>
                <span v-for='(value, key, index) in valueObj'>{{valueObj[key]}}</span>
              </div>
          </div>
        </scroller>
        <a href="javascript:;" v-on:click='hideDiscribe'><x-button type="warn" class='button_close' >完成</x-button>
        </a>
     </div>
...
<div class='store_info'>
   <div class='store_info_title'>
      <div class='store_info_left'>
          <div class='store_info_left_img'>
            <img src='../../assets/images/girl.jpg'/>
          </div>
      </div>
      <div class='store_info_right'>
        <span class='store_name'>{{this.storeInfo.store_name}}</span>
      </div>
   </div>
   <div class='store_info_title_credit'>
      <div class='store_info_title_credit_number'>
        <p class='number'>18</p>
        <p class='all'>全部宝贝</p>
      </div>
      <div class='store_info_title_credit_follow'>
        <p class='number'>1.4万</p>
        <p class='all'>关注人数</p>      
      </div>
      <div class='store_info_title_credit_detail'>
         <p class='all'>描述
            <span class='credit'>{{this.storeDesccredit.credit}}</span> 
            <span class='percent'>{{this.storeDesccredit.percent_text}}</span>
         </p>
         <p class='all'>服务 
            <span class='credit'>{{this.storeServicecredit.credit}}</span> 
            <span class='percent'>{{this.storeServicecredit.percent_text}}</span>
         </p>  
         <p class='all'>物流 
            <span class='credit'>{{this.storeDeliverycredit.credit}}</span> 
            <span class='percent'>{{this.storeDeliverycredit.percent_text}}</span>
         </p>
      </div>
   </div>
</div>
...
import {Group, XHeader, Rater, XNumber, Toast, Cell, Scroller, XButton} from 'vux'
import { swiper, swiperSlide } from 'vue-awesome-swiper'
import { requestGetGoodsComment, requestGetGoodsDetail } from '../../api/api'
export default {
  components: {
    swiper,
    swiperSlide,
  ...
  methods: {
    ...
    getGoodsInfo () {
      requestGetGoodsDetail(this.$route.params.id).then(data => {
        this.goodsInfo = data.datas.goods_info
        this.goodsHairInfo = data.datas.goods_hair_info
        this.storeInfo = data.datas.store_info
        this.storeDesccredit = data.datas.store_info.store_credit.store_desccredit
        this.storeServicecredit = data.datas.store_info.store_credit.store_servicecredit
        this.storeDeliverycredit = data.datas.store_info.store_credit.store_deliverycredit
      })
    },
    cartMask () {
...
.title_d .market_price,.text_gray.market_price,.goods_hair_info{
  font-size: 12px;
  display:block;
  padding-left: 4px;
}
.title_d .market_price span{
...
.scroll_box_info span:nth-child(1){
    width:80%;
}
.scroll_box_info span:nth-child(2){
    width:20%;
}
/*店铺信息*/
.store_info{
...
.store_info_left .store_info_left_img{
  width:60px;
  height:60px;
  border-radius:100%;
  overflow:hidden;
...
.store_info_right .store_name{
  color:#333333;
  font-size:18px;
}
.store_info_title_credit{
  display:flex;

}
.store_info_title_credit div{
  width:33%;
}
.store_info_title_credit{
  text-align:center;
...
.store_info_title_credit_follow{
  border-right:1px solid #dddddd;
  border-left:1px solid #dddddd;
}
.store_info_title_credit .number{
  font-size:16px;
}
.store_info_title_credit .all{
  font-size:14px;
  color:#878787;
}
.store_info_title_credit_detail span.credit{
  color:#ff5000;
}
.store_info_title_credit_detail span.percent{
  color:#FFF;
  display:inline-block;
  width:18px;
  height:18px;
...
.goods_hair_info span{
  width:32%;
  display:inline-block;
}
.goods_hair_info span:nth-child(2){
  text-align:center;
}
.goods_hair_info span:nth-child(3){
  text-align:right;
}
...
/*商品详情底部*/
.goods-detail-foot { position: fixed; z-index: 1; left: 0; right: 0; bottom: 0; height: 44px; filter:progid:DXImageTransform.Microsoft.gradient(enabled='true',startColorstr='#F2F5F5F5', endColorstr='#F2F5F5F5');background:rgba(245,245,245,0.95);}
...
.goods-option-foot .otreh-handle { float: left; width: 35%; font-size: 0;border-top: solid 0.05rem #DDD;}
.goods-detail-foot .otreh-handle a,
.goods-option-foot .otreh-handle a { position: relative; z-index: auto; display: inline-block; height: 43px; text-align: center; color: #333; }
.goods-detail-foot .otreh-handle a.kefu,
.goods-option-foot .otreh-handle a.kefu { width: 47%;}
.goods-detail-foot .otreh-handle a.cart,
.goods-option-foot .otreh-handle a.cart { width: 47%; border-left: solid 0.05rem #DDD;}
.goods-detail-foot .otreh-handle a i,
.goods-option-foot .otreh-handle a i { display: inline-block; width: 0.9rem; height: 20px; margin: 0.15rem auto 0.00rem auto; background-repeat: no-repeat; background-position: 50% 50%; background-size: 80%; opacity: 0.65;}
